...
const VisitReportForm = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (res && res.success) {
        addToast('Site Visit Consultation Report submitted successfully!', 'success');
        navigate('/designer');
      }
    } catch (err) {
      console.error('Report submission error:', err);
      addToast(err.message || 'Failed to submit report. Ensure database integrity.', 'error');
    } finally {
      setSubmitting(false);
    }
  };
  ...
  return (
    <div className="visit-report-page max-w-5xl mx-auto flex flex-col gap-6">
      {/* Page Header */}
      <div className="flex justify-between items-center border-b pb-4 border-border">
        <div>
          <h1 className="heading-xl text-cream m-0" style={{ fontFamily: 'var(--font-heading)' }}>
            Consultation Report: {booking.booking_ref}
          </h1>
          <p className="text-muted text-sm m-0">Submit site observations, dimensions, and styling suggestions below.</p>
        </div>
        <button className="btn btn-secondary" onClick={() => navigate('/designer')}>
          &larr; Back to Dashboard
        </button>
      </div>

      <form onSubmit={handleSubmit} className="flex flex-col gap-6">
        {/* Row 1: Client Overview Card */}
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6" style={{ display: 'grid', gridTemplateColumns: '2fr 1fr', gap: '24px' }}>
          ...
          <div className="flex flex-col gap-4">
            <div className="card bg-surface p-6 border border-border rounded-xl flex flex-col gap-4">
              <h3 className="heading-md text-gold border-b pb-2 mb-2 m-0" style={{ fontFamily: 'var(--font-heading)' }}>Observations & Suggestions</h3>
              
              <div className="form-group">
                <label className="form-label" htmlFor="obs-input">Site Observations</label>
                <textarea
                  id="obs-input"
                  className="form-textarea"
                  rows="4"
                  placeholder="Describe the current status of the walls, flooring, layout configurations..."
                  value={observations}
              ...
              <div className="form-group">
                <label className="form-label" htmlFor="design-input">Design Suggestions</label>
                <textarea
                  id="design-input"
                  className="form-textarea"
                  rows="3"
                  placeholder="Furniture layouts, color theme options..."
              ...
              <div className="form-group">
                <label className="form-label" htmlFor="material-input">Material Recommendations</label>
                <textarea
                  id="material-input"
                  className="form-textarea"
                  rows="3"
                  placeholder="Wood options, tiling recommendations, lighting details..."
          ...
          <div className="flex flex-col gap-6">
            <div className="card bg-surface p-6 border border-border rounded-xl flex flex-col gap-4">
              <h3 className="heading-md text-gold border-b pb-2 mb-2 m-0" style={{ fontFamily: 'var(--font-heading)' }}>Report Metadata</h3>
              ...
              <div className="form-group">
                <label className="form-label" htmlFor="report-budget">Estimated Project Budget (₹)</label>
                <input
                  id="report-budget"
                  type="number"
                  className="form-input"
              ...
              <div className="form-group">
                <label className="form-label" htmlFor="followup-input">Follow-up Action Notes</label>
                <textarea
                  id="followup-input"
                  className="form-textarea"
                  rows="3"
                  placeholder="Quotations revised schedule, second site visit details..."
                  value={followUpNotes}
        ...
        {/* Row 3: Room Measurements Dynamic Table */}
        <div className="card bg-surface p-6 border border-border rounded-xl">
          <div className="flex justify-between items-center mb-4 border-b pb-2 border-border">
            <h3 className="heading-md text-gold m-0" style={{ fontFamily: 'var(--font-heading)' }}>Site Dimensions & Measurements</h3>
        ...
        {/* Row 4: Summary Box with AI generate tool */}
        <div className="card bg-surface p-6 border border-border rounded-xl">
          <div className="flex justify-between items-center mb-4 border-b pb-2 border-border">
            <h3 className="heading-md text-gold m-0" style={{ fontFamily: 'var(--font-heading)' }}>Executive Consultation Summary</h3>
            <button
              type="button"
          ...
          <div className="form-group mb-0">
            <textarea
              className="form-textarea"
              rows="5"
              placeholder="This summary is printable on the client's PDF sheet. Write a professional consultation summary or click 'Auto-Generate AI Summary' above..."
              value={summary}
        ...
        <div className="flex justify-end gap-4">
          <button type="button" className="btn btn-secondary py-3 px-6" onClick={() => navigate('/designer')} disabled={submitting}>
            Cancel
          </button>
          <button type="submit" className="btn btn-primary py-3 px-8 font-semibold" disabled={submitting}>
            {submitting ? 'Submitting Report...' : 'Submit Consultation Report'}
          </button>
        </div>
      </form>
    </div>
